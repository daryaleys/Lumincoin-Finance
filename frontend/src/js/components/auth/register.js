...
export class Register {
  constructor(openNewRoute) {
    ...
    this.repeatPasswordInputElement =
  ...
  validateForm() {
    ...
    if (
      ...
      this.emailInputElement.value.match(
        /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/
      )
    ) {
    ...
    if (
      ...
      this.passwordInputElement.value.match(
    ...
    if (
      ...
      this.repeatPasswordInputElement.value === this.passwordInputElement.value
    ) {
  ...
  async register() {
    ...
    if (this.validateForm()) {
      ...
      const result2 = await Requests.request(
        "/login",
        "POST",
        false,
        loginBody
      ...
      UserInfo.setUserInfo(
        result2.tokens.accessToken,
        result2.tokens.refreshToken,
        result2.user.id,
        result2.user.name,
        result2.user.lastName
      );
