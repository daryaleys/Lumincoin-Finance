...
export class Expenses {
	...
	private async getCategories(): Promise<void> {
		const result: CommonResultType | CategoryType[] = await Requests.request("/categories/expense", "GET", true);
		...
		if (targetElement) {
			...
			if (this.deleteCategoryButton) {
				this.deleteCategoryButton.onclick = async () => {
					const result: CommonResultType = await Requests.request("/categories/expense/" + categoryId, "DELETE", true);

					if (result.error) {
						return alert("Возникла ошибка при удалении категории. Пожалуйста, обратитесь в поддержку");
					}
